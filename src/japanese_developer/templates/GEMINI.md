...
**コミットメッセージのルール:**
- 形式: `何をしたか: なぜやったか`
- 「なぜやったか」は、**ユーザーとの対話内容から意図を読み取って生成**すること。
- ユーザーの依頼内容・背景・目的を要約して含める。
- 例:
  - `ログインにバリデーション追加: 空欄送信でサーバーエラーになる問題への対策`
  - `APIレスポンスをキャッシュ化: 表示速度が遅いとユーザーから報告があったため`
  - `画像アップロード機能追加: ユーザーがプロフィール画像を設定したいとの要望`
  - `CSSをdarkmode対応に修正: 全画面ダークモード統一方針に基づく`
- 「修正しました」「変更しました」のような中身のないメッセージは禁止。
...
ユーザーが「エラーが出た」「動かない」「おかしい」等の報告をしてきた場合、
いきなり修正を試みてはならない。**まず以下の情報を収集すること。**

### 必ず確認する5項目

1. **エラーメッセージ全文** — 「エラーが出た」だけでは診断不能。ターミナル出力・ブラウザコンソール・ログファイルの該当箇所を貼ってもらう
2. **何をしようとしていたか** — どんな操作をした直後にエラーが出たか
3. **実行したコマンド** — 正確なコマンド文字列（手打ちだと打ち間違いの可能性もある）
4. **いつから発生しているか** — 「最初から」なのか「さっきまで動いてた」のかで原因が変わる
5. **最近変更したこと** — コードの変更、パッケージの追加/更新、設定ファイルの編集など

### エラーメッセージの探し方を教える

素人はエラーメッセージがどこに出ているか分からないことが多い。以下を案内すること：

- **ターミナル**: コマンド実行後の赤い文字やTraceback/Error行を探す
- **ブラウザ**: F12 → コンソールタブの赤い行
- **ログファイル**: `npm-debug.log`、`logs/`配下、`/tmp/`配下など
- **git**: `git status` や `git diff` で意図しない変更がないか

### 対応フロー

1. 上記5項目を聞く（既に情報があれば省略可）
2. エラーメッセージを読み、**原因を日本語で平易に説明する**（例: 「ポート8080が既に別のプロセスに使われている」）
3. 修正方法を提案する前に、原因の説明に対してユーザーが理解したか確認する
4. 修正を実行する
5. 修正後、同じ操作をして問題が解消したか確認する
